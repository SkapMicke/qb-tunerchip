...
RegisterNUICallback('save', function(data)
    QBCore.Functions.TriggerCallback('qb-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            ...
            setVehData(veh, data)
            QBCore.Functions.Notify('TunerChip v1.05: Fordonet inställt!', 'error')
...
RegisterNetEvent('qb-tunerchip:client:TuneStatus', function()
    ...
    if vehModel ~= 0 then
        QBCore.Functions.TriggerCallback('qb-tunerchip:server:GetStatus', function(status)
            if status then
                QBCore.Functions.Notify('Fordonet har blivit inställt', 'success')
            else
                QBCore.Functions.Notify('Fordonet har inte blivit inställt', 'error')
            end
        end, plate)
    else
        QBCore.Functions.Notify('Inga fordon nära', 'error')
    end
end)
...
RegisterNUICallback('reset', function(data)
    ...
    local veh = GetVehiclePedIsUsing(ped)
    resetVeh(veh)
    QBCore.Functions.Notify('TunerChip v1.05: Fordonet har blivit återställt!', 'error')
end)
...
RegisterNetEvent('qb-tunerchip:client:openChip', function()
    ...
    if inVehicle then
        QBCore.Functions.Progressbar("connect_laptop", "Tunerchip v1.05: Fordonet har blivit återställt!", 2000, false, true, {
        ...
        QBCore.Functions.Notify("Du är inte i något fordon", "error")
    end
end)
