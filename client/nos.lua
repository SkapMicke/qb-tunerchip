    ...
    if not NitrousActivated then
        if IsInVehicle and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
            if GetPedInVehicleSeat(veh, -1) == ped then
                QBCore.Functions.Progressbar("use_nos", "Skruvar upp lustgasen...", 1000, false, true, {
                ...
                QBCore.Functions.Notify("Du kan inte göra detta från detta sätet!", "error")
            end
        else
            QBCore.Functions.Notify('Du är inte i ett fordon', 'error')
        end
    else
        QBCore.Functions.Notify('Du har redan lustgas inkopplat', 'error')
    end
end)
